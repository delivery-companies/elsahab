...
import { bulkForwardOrderService } from "@/services/editOrder";
...
export const ForwardOrdersToCompany = () => {
  ...
  const { mutate: editOrder, isLoading } = useMutation({
    mutationFn: (data: { companyId: number; ordersIDs: string[] }) =>
      bulkForwardOrderService({ data }),
    onSuccess: () => {
      close();
  ...
  const handleSubmit = () => {
    if (selectedCompany) {
      editOrder({
        ordersIDs: selectedOrders.map((o) => o.id),
        companyId: +selectedCompany,
      });
      toast.success("تم اسناد الطلبات بنجاح");
    }
  };
